...
- Photos (`/photos`): the JPEGs live in the `nodeconf-eu-photos` R2 bucket (binding `PHOTOS`), not in the repo, keyed `<album date>/<file>.jpg`. `app/photos/page.tsx` is a server component that lists the bucket per request (edge-cached ~60s) via `app/photos/photo-store.ts`, so uploads appear without a redeploy; `app/photos/gallery.tsx` is the client grid + PhotoSwipe lightbox. Albums are declared by `content/photos/<date>/_album.md` (title/date/weekday, plus `photographer`/`photographerUrl` for the credit shown under the album, in the lightbox and in the app), loaded by `app/photos/albums.ts` — a bucket prefix without one is not shown. `app/photos/pick.json` returns one photo (`?mode=latest` by R2 upload time, or `?mode=random`) for the attendee app's photo-wall card. Dimensions are parsed from each JPEG header with `image-size` and cached in `_index/<date>.json`; thumbnails are made on first request by the `IMAGES` binding and stored under `_thumbs/`. Originals are served by `app/photos/full/[...key]`, thumbnails by `app/photos/thumb/[...key]`, both with `?v=<etag>` immutable caching. Local dev uses miniflare's R2/Images emulation (seed with `wrangler r2 object put nodeconf-eu-photos/<key> --file … --local`); its Images emulation ignores `fit`, so local thumbnails are letterboxed squares.
- `scripts/upload-photos.ts` (`npm run photos:upload -- <date> <files|folders>`): the photographer's upload tool. Dependency-free (Node built-ins, hand-rolled SigV4, tested in `test/upload-photos.test.ts`) so it can be run standalone. Authenticates to R2's S3 API with `R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY` (env or `./.env`) from a bucket-scoped "Object Read & Write" R2 API token; skips files already uploaded; `--delete` removes photos.

- Attendee app (`/app`): a phone-sized, installable program. `app/app/page.tsx` sets the metadata/viewport and links `public/app.webmanifest` (scope `/app`, so only this page is installable); `app/app/attendee-app.tsx` is the client UI (day tabs, now/next, starred sessions in `localStorage` under `nodeconf-app-stars`, Watch Live, install prompt); `app/app/schedule.ts` holds the pure time logic (Bologna wall clock, now/next), tested in `test/attendee-schedule.test.ts`. Data comes from `app/program-data.ts`, so program edits flow through automatically. `public/sw.js` provides offline support (network-first `/app` with a 3.5 s fallback, cache-first `/assets/*`) and is only registered in production builds — test offline with `npm run build && npx vite preview` (runs the built Worker in workerd; `npm run start` cannot load `cloudflare:workers`), not `npm run dev`. Bump `VERSION` in `sw.js` when changing its caching rules. Icons live in `public/app-icons/`. Livestream constants shared with the home page are in `app/event.ts`.
